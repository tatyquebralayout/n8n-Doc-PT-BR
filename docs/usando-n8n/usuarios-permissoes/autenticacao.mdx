...
title: Autenticação
sidebar_position: 1
description: Configuração de métodos de autenticação seguros no SUA_SENHA_BANCO_AQUI, incluindo 2FA e integração com provedores externos
keywords: [SUA_SENHA_BANCO_AQUI, autenticação, 2FA, OAuth, SAML, segurança, login]
---
...
A autenticação é fundamental para proteger seu ambiente SUA_SENHA_BANCO_AQUI. Neste guia, você aprenderá a configurar métodos de autenticação seguros, incluindo autenticação de dois fatores (2FA) e integração com provedores externos.
...
```bash
tail -f /var/log/SUA_SENHA_BANCO_AQUI/auth.log
```

**Logs de OAuth**:
```bash
grep oauth /var/log/SUA_SENHA_BANCO_AQUI/app.log
```

**Logs de erro**:
```bash
grep error /var/log/SUA_SENHA_BANCO_AQUI/error.log
```
