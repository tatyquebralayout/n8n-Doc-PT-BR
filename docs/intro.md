...
- **Demonstração Híbrida** - Integração reStructuredText + Sphinx + Docusaurus

---

## <ion-icon name="flask-outline" style={{ fontSize: '24px', color: '#ea4b71' }}></ion-icon> Demonstração: Integração Híbrida reStructuredText + Sphinx + Docusaurus

Esta seção demonstra como podemos integrar funcionalidades avançadas do reStructuredText e Sphinx mantendo o Docusaurus como tema principal.

### <span style={{color: '#ea4b71'}}>Funcionalidades Híbridas Disponíveis</span>

:::info Funcionalidade Híbrida
Esta seção usa componentes customizados que permitem sintaxe reStructuredText dentro do Markdown do Docusaurus.
:::

#### 1. **Sintaxe reStructuredText Integrada**
...
.. code-block:: python
   ...
   def processar_dados_n8n(dados):
       """Processa dados do n8n usando Python."""
       resultado = []
       for item in dados:
...
#### 2. **Diretivas Sphinx Customizadas**

```rst
.. n8n-workflow:: exemplo-workflow
   :title: Workflow de Processamento de Leads
   :description: Automatiza o processamento de leads do formulário
   :nodes: 5
   :executions: 1000/month
   
   .. workflow-step:: webhook-trigger
      :description: Recebe dados do formulário
      :config: { "method": "POST", "path": "/leads" }
   
   .. workflow-step:: validate-data
      :description: Valida email e telefone
      :code: |
   ...
   .. workflow-step:: crm-update
      :description: Atualiza CRM
      :service: salesforce
      :config: { "operation": "create", "resource": "lead" }
   
   .. workflow-step:: slack-notification
      :description: Notifica equipe
      :channel: "#leads"
      :template: "Novo lead: {email} - {company}"
   
   .. workflow-step:: email-followup
      :description: Envia email de boas-vindas
      :template: "welcome-email"
      :delay: "5 minutes"
```

#### 3. **Tabelas Avançadas com Sphinx**

```rst
.. table:: Comparação de Plataformas de Automação
   :widths: 20 25 25 30
   :align: center
   
   +----------------+------------------+------------------+------------------+
   | Característica | n8n              | Zapier           | Power Automate   |
   +================+==================+==================+==================+
   | Custo 100K     | R$ 26-260/mês   | R$ 100-160/mês  | R$ 78-1.120/mês |
   | execuções/mês  | (self-hosted)    | (limite 100K)   | (limite 100K)   |
   +----------------+------------------+------------------+------------------+
   | Código custom. | ✅ JavaScript    | ❌ Limitado      | ❌ Limitado      |
   |                | ✅ Python        |                  |                  |
   +----------------+------------------+------------------+------------------+
   | Self-hosted    | ✅ Controle total| ❌ Cloud only    | ❌ Cloud only    |
   +----------------+------------------+------------------+------------------+
   | Integrações    | 400+ nativas     | 5000+ (muitas   | 400+ Microsoft   |
   |                | + comunidade     | pagas)           | + limitadas      |
   +----------------+------------------+------------------+------------------+
```

#### 4. **Diagramas Técnicos com Sphinx**

```rst
.. graphviz:: workflow-n8n
   :caption: Arquitetura de Workflow n8n
   :align: center
   ...
   digraph n8n_workflow {
       ...
       subgraph cluster_error {
           ...
           validate -> error_handler [style=dashed];
           crm -> retry [style=dashed];
       }
   }
```

#### 5. **Documentação de API Integrada**

```rst
.. http:post:: /api/v1/workflows/{id}/execute
   :synopsis: Executa um workflow específico
   
   Executa um workflow n8n pelo ID fornecido.
   ...
   :>json object success: Status da execução
   :>json string executionId: ID da execução
   :>json object result: Resultado do workflow
...
#### 6. **Índice Automático com Sphinx**
...
### <span style={{color: '#ea4b71'}}>Benefícios da Integração Híbrida</span>

:::tip Vantagens da Abordagem Híbrida
- **Markdown simples** para conteúdo básico
...
### <span style={{color: '#ea4b71'}}>Como Usar</span>

Para usar estas funcionalidades híbridas em outras páginas:

1. **Incluir diretivas RST** dentro de blocos de código markdown
2. **Usar componentes customizados** para renderização
3. **Processar automaticamente** via build pipeline
4. **Manter compatibilidade** com Markdown puro

:::note Implementação
Esta funcionalidade é implementada através de componentes React customizados que processam sintaxe RST e Sphinx, mantendo a compatibilidade com o Docusaurus.
:::
