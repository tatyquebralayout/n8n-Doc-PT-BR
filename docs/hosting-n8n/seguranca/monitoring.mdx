...
#ionicicon-nameanalytics-outline-style-fontsize-24px color: '#cor-ea4b71-mtricas-essenciais

#mtricas-de-sistema

#cpu-e-memria-bash
#bin/bash
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-system-metricssh-echo "=== Métricas do Sistema ==="

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-cpu-cpuusagetop -bn1 | grep "Cpu(s)" | awk '{print $2}' | cut -d'%' -f1)
echo "CPU Usage: ${CPU_USAGE}%"

#ionicicon-namesparkles-outline-style-fontsize-24px color: '#cor-ea4b71-memria-memoryusagefree | grep Mem | awk '{printf("%.2f", $3/$2 * 100.0)}')
echo "Memory Usage: ${MEMORY_USAGE}%"

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-disco-diskusagedf / | tail -1 | awk '{print $5}' | cut -d'%' -f1)
echo "Disk Usage: ${DISK_USAGE}%"

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-load-average
LOAD_AVG=$(uptime | awk -F'load average:' '{print $2}' | awk '{print $1}' | cut -d',' -f1)
echo "Load Average: $LOAD_AVG"
```

#mtricas-de-rede-bash
#bin/bash
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-network-metricssh-echo "=== Métricas de Rede ==="

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-conexes-ativas
ACTIVE_CONNECTIONS=$(netstat -an | grep ESTABLISHED | wc -l)
echo "Active Connections: $ACTIVE_CONNECTIONS"

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-conexes-n8n
N8N_CONNECTIONS=$(netstat -an | grep :5678 | grep ESTABLISHED | wc -l)
echo "n8n Connections: $N8N_CONNECTIONS"

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-trfego-de rede
...
#mtricas-n8n-bash
#bin/bash
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-n8n-metricssh-n8nurlhttplocalhost5678
API_KEY="sua_api_key"

echo "=== Métricas n8n ==="

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-status-da aplicação
...
#ionicicon-namegit-branch-outline-style-fontsize-24px color: '#cor-ea4b71-nmero-de workflows
WORKFLOW_COUNT=$(curl -s -H "X-N8N-API-KEY: $API_KEY" \
  "$N8N_URL/api/v1/workflows" | jq '. | length')
echo "Total Workflows: $WORKFLOW_COUNT"

#ionicicon-namegit-branch-outline-style-fontsize-24px color: '#cor-ea4b71-workflows-ativos
...
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-execues-nas últimas 24h
...
#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-tempo-de resposta da API
API_RESPONSE_TIME=$(curl -o /dev/null -s -w "%{time_total}" \
  http://localhost:5678/api/v1/workflows)
echo "API Response Time: ${API_RESPONSE_TIME}s"

#ionicicon-namegrid-outline-style-fontsize-24px color: '#cor-ea4b71-tempo-de resposta da UI
UI_RESPONSE_TIME=$(curl -o /dev/null -s -w "%{time_total}" \
  http://localhost:5678/)
echo "UI Response Time: ${UI_RESPONSE_TIME}s"

#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-uso-de memória do Node.js
...
#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-uso-de CPU do Node.js
...
#ionicicon-namecolor-palette-outline-style-fontsize-24px color: '#cor-ea4b71-sistema-de Alertas

#configurao-de-alertas

#script-de-monitoramento-bash
#bin/bash
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-n8n-monitoringsh
...
#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-funo-para enviar alerta
send_alert() {
    local message="$1"
    local severity="$2"
    
    #slack-curl-x-post-webhookurl \
      -H "Content-type: application/json" \
      -d "{\"text\":\"$severity $message\"}"
    
    #email-echo-message-mail -s "n8n Alert: $severity" $EMAIL_ALERTS
    
    #log-echo-date-severity $message" >> /var/log/n8n/alerts.log
}
...
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-verificar-disco
DISK_USAGE=$(df / | tail -1 | awk '{print $5}' | cut -d'%' -f1)
if [ $DISK_USAGE -gt $ALERT_THRESHOLD_DISK ]; then
    send_alert "Disk usage is ${DISK_USAGE}%" "🚨"
fi

#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-verificar-n8n
...
#ionicicon-nametime-outline-style-fontsize-24px color: '#cor-ea4b71-verificar-tempo de resposta
...
#alertas-de-horrio-bash
#bin/bash
#ionicicon-nametime-outline-style-fontsize-24px color: '#cor-ea4b71-time-based-alertssh

#ionicicon-namekey-outline-style-fontsize-24px color: '#cor-ea4b71-configuraes-currenthourdate +%H)
CURRENT_DAY=$(date +%u)  #id-1monday-7sunday-businesshoursstart9-businesshoursend18

#ionicicon-nametime-outline-style-fontsize-24px color: '#cor-ea4b71-verificar-se está em horário comercial
if [ $CURRENT_DAY -ge 1 ] && [ $CURRENT_DAY -le 5 ]; then
    if [ $CURRENT_HOUR -ge $BUSINESS_HOURS_START ] && [ $CURRENT_HOUR -lt $BUSINESS_HOURS_END ]; then
        #em-horrio-comercial-alertas mais rigorosos
        ALERT_THRESHOLD_CPU=70
        ALERT_THRESHOLD_MEMORY=80
        ALERT_THRESHOLD_ERROR_RATE=2
    else
        #fora-do-horrio-comercial alertas mais flexíveis
...
#ionicicon-namechevron-forward-outline-style-fontsize-24px color: '#cor-ea4b71-dashboards-de Observabilidade

#grafana-dashboard

#configurao-do-grafana-yaml
#ionicicon-namecloud-outline-style-fontsize-24px color: '#cor-ea4b71-docker-composeyml-para Grafana
version: '3.8'
...
#configurao-prometheus-yaml
#ionicicon-namesparkles-outline-style-fontsize-24px color: '#cor-ea4b71-prometheusprometheusyml-global
  scrape_interval: 15s
...
#configurao-datadog-yaml
#ionicicon-nameanalytics-outline-style-fontsize-24px color: '#cor-ea4b71-datadog-agentyaml-apiversion v1
kind: ConfigMap
metadata:
  name: datadog-config
data:
...
#custom-metrics-javascript-n8n-datadog-metricsjs
...
#configurao-n8n-bash
#ionicicon-namesettings-outline-style-fontsize-24px color: '#cor-ea4b71-configurao-de logs estruturados
N8N_LOG_LEVEL=info
N8N_LOG_FORMAT=json
...
#ionicicon-namecloud-outline-style-fontsize-24px color: '#cor-ea4b71-docker-compose-elkyml-version '3.8'
...
#logstash-pipeline-ruby
#ionicicon-namedocument-outline-style-fontsize-24px color: '#cor-ea4b71-logstashpipelinen8nconf-input {
  beats {
    port => 5044
  }
}
...
#ionicicon-namegit-network-outline-style-fontsize-24px color: '#cor-ea4b71-integrao-com Ferramentas APM

#new-relic

#configurao-new-relic-javascript newrelic.js
'use strict';
...
#configurao-appdynamics-javascript-appdynamicsjs
...
#ionicicon-namechevron-forward-outline-style-fontsize-24px color: '#cor-ea4b71-checklist-de Monitoramento

#mtricas-mtricas de sistema configuradas
- [ ] Métricas de aplicação implementadas
- [ ] Métricas de negócio definidas
- [ ] Coleta automática configurada
- [ ] Retenção de dados definida

#alertas-alertas críticos configurados
...
#dashboards-dashboard principal criado
- [ ] Dashboards específicos por equipe
- [ ] Métricas em tempo real
- [ ] Histórico de dados
- [ ] Exportação de relatórios

#logs-logs estruturados configurados
