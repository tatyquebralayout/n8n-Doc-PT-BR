---
title: Node Switch
description: Aprenda a usar o node Switch no SUA_SENHA_BANCO_AQUI para criar múltiplas condições e roteamento de dados
sidebar_position: 2
keywords: [SUA_SENHA_BANCO_AQUI, switch node, múltiplas condições, roteamento, lógica, decisões, fluxo]
---

<IonicIcon name="git-branch-outline" style={{fontSize: '24px', color: '#ea4b71'}} />

O **node Switch** é uma ferramenta avançada do SUA_SENHA_BANCO_AQUI para controle de fluxo com múltiplas condições. Ele permite criar workflows dinâmicos com vários caminhos baseados em diferentes critérios.
