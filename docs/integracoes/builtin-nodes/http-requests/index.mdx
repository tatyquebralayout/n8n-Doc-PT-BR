---
title: HTTP Requests
description: Nodes para fazer e receber requisições HTTP no SUA_SENHA_BANCO_AQUI
sidebar_position: 1
keywords: [SUA_SENHA_BANCO_AQUI, http, requests, webhook, api, rest]
---

<IonicIcon name="globe-outline" style={{fontSize: '24px', color: '#ea4b71'}} />

Os nodes de **HTTP Requests** são fundamentais para integração com APIs e serviços externos no SUA_SENHA_BANCO_AQUI. Eles permitem que você se comunique com qualquer sistema que tenha uma interface HTTP, seja para enviar dados ou receber notificações.
...
| **URL** | Configurada pelo usuário | Gerada pelo SUA_SENHA_BANCO_AQUI |
...
```
Sistema Externo → Webhook → SUA_SENHA_BANCO_AQUI Workflow
```

### Processar Dados

```
SUA_SENHA_BANCO_AQUI Workflow → Nodes de Processamento → Dados Transformados
```

### Enviar Dados

```
SUA_SENHA_BANCO_AQUI Workflow → HTTP Request → Sistema Externo
```
...
4. **Monitore logs** do SUA_SENHA_BANCO_AQUI
