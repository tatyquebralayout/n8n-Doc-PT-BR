...
#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-expression-node
...
#ionicicon-namesparkles-outline-style-fontsize-24px color: '#cor-ea4b71-sintaxe-bsica

#id-1-estrutura-das-expresses-javascript
{{ expressão JavaScript }}
```

#id-2-exemplos-simples-javascript $json.nome }}                    // Acessar campo 'nome'
{{ $json.preco * 1.1 }}            // Calcular com 10% de acréscimo
{{ $json.ativo ? 'Sim' : 'Não' }}  // Condicional ternário
{{ new Date().toISOString() }}     // Data/hora atual
```

#ionicicon-namebulb-outline-style-fontsize-24px color: '#cor-ea4b71-variveis-disponveis
...
#exemplos-de-uso-javascript Acessar dados do item atual
{{ $json.email }}
...
#ionicicon-namesettings-outline-style-fontsize-24px color: '#cor-ea4b71-casos-de Uso Práticos**

#id-1-http-request-url Dinâmica**
...
#id-2-email-assunto-dinmico

**Configuração:**

```
To: {{ $json.email }}
Subject: {{ 'Pedido #jsonnumeropedido + $json.status }}
Body: |
  Olá {{ $json.nome }},
  
  Seu pedido #jsonnumeropedido-foi-jsonstatus }}.
...
#id-3-set-node-campos Calculados**
...
#id-4-if-node-condies Dinâmicas**
...
#id-5-schedule-trigger-horrio Dinâmico**
...
#ionicicon-namecode-slash-outline-style-fontsize-24px color: '#cor-ea4b71-expresses-avanadas

#manipulao-de-arrays-javascript Acessar primeiro elemento
...
#manipulao-de-strings-javascript Capitalizar
...
#clculos-matemticos-javascript-clculo com desconto
...
#condicionais-complexas-javascript-mltiplas condições
{{ 
  $json.idade < 18 ? 'menor' : 
  $json.idade < 25 ? 'jovem' : 
...
#formatao-de-datas-javascript Data atual formatada
...
#ionicicon-nameshield-checkmark-outline-style-fontsize-24px color: '#cor-ea4b71-validaes-e Tratamento de Erros**

#validao-de-campos-javascript Verificar se campo existe
...
#tratamento-de-erros-javascript Verificar se é número
{{ 
...
#ionicicon-namesettings-outline-style-fontsize-24px color: '#cor-ea4b71-exemplos-por Node**

#http-request-node-javascript URL dinâmica
...
#email-node-javascript-assunto personalizado
...
Seu pedido #jsonnumero-foi-jsonstatus-detalhes Produto: ${$json.produto}
- Valor: R$ ${$json.valor.toFixed(2)}
...
#set-node-javascript-campo calculado
...
#if-node-javascript-condio simples
{{ $json.valor > 1000 }}
...
#limitaes-das-expresses-escopo limitado**: Apenas uma linha de código
...
#boas-prticas-javascript Bom: Expressão simples e clara
{{ $json.nome + ' ' + $json.sobrenome }}
...
#cor-debugging-javascript-usar-consolelog para debug (aparece nos logs do n8n)
