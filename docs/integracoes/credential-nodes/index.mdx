...
keywords: [SUA_SENHA_BANCO_AQUI, credential nodes, autenticação, credenciais, oauth]
---

<IonicIcon name="key-outline" style={{fontSize: '24px', color: '#ea4b71'}} />

As **Credenciais** são informações de autenticação que permitem que o SUA_SENHA_BANCO_AQUI se conecte com APIs e serviços externos de forma segura. Elas são essenciais para qualquer integração que requeira autenticação.

## O que são Credenciais

Credenciais são dados sensíveis (como chaves de API, tokens, usuários e senhas) que são armazenados de forma segura no SUA_SENHA_BANCO_AQUI e usados para autenticar com serviços externos.
...
Credenciais específicas para integrações suportadas pelo SUA_SENHA_BANCO_AQUI:
...
### Configurar Credenciais Predefinidas
...
### Configurar Credenciais Genéricas
...
4. Adicionar token no SUA_SENHA_BANCO_AQUI
...
- [Expressões SUA_SENHA_BANCO_AQUI](/logica-e-dados/expressoes) - Usar credenciais dinamicamente
