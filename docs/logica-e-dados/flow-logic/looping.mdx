...
#ionicicon-namerepeat-outline-style-fontsize-24px color: '#cor-ea4b71-looping-no n8n
...
#o-que-looping-looping é o processo de **repetir operações** sobre um conjunto de dados, permitindo:
...
#tipos-de-loops-1-for Loop**: Iteração sobre array com índice
...
#id-2-code-node-para-loops Customizados
...
#ionicicon-namecode-outline-style-fontsize-24px color: '#cor-ea4b71-padres-de Loop

#id-1-processamento-de-array-processa cada elemento de um array.
...
#id-2-retry-com-backoff-tenta uma operação múltiplas vezes com delays crescentes.
...
#id-3-polling-verifica-status-periodicamente até uma condição ser atendida.
...
#id-1-processamento-de-pedidos-em Lote
...
#id-2-sincronizao-de-dados-cenrio Sincronizar dados entre sistemas diferentes.
...
#id-3-processamento-paralelo-cenrio-executar múltiplas operações simultaneamente.
...
#loops-dinmicos-javascript-tamanho do batch baseado em dados
...
#loops-condicionais-javascript-loop baseado em condição
const dados = $json;
const resultados = [];
...
#loops-com-controle-javascript Loop com break condicional
const dados = $json;
const resultados = [];
...
#loop-com-error-handling-javascript
...
#loop-com-retry-javascript Loop com retry para itens que falham
const dados = $json;
...
#boas-prticas-1-otimize-batch Size**
...
#exemplo-de-otimizao-javascript Loop otimizado com rate limiting
const dados = $json;
const resultados = [];
...
#problemas-comuns-erro-timeout Reduza tamanho do batch
...
#cor-debugging-javascript-loop-com debugging
const dados = $json;
const resultados = [];
const debug = {
...
#ionicicon-namedocument-text-outline-style-fontsize-24px color: '#cor-ea4b71-referncias **[Split In Batches](../../integracoes/builtin-nodes/data-processing/split-in-batches)** - Nó específico para loops
...
> <IonicIcon name="bulb-outline" style={{ fontSize: '18px', color: '#cor-ea4b71-dica-use loops estrategicamente para processar dados em lote e criar workflows dinâmicos. Lembre-se de sempre implementar rate limiting e tratamento de erros para garantir robustez e performance.
